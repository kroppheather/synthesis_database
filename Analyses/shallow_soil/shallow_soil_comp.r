...
for(i in 1:dim(compI)[1]){
	...
	for(j in 1:dim(datVI)[1]){
		...
		mtext(paste("p=",round(summary(fitV)$coefficients[2,4],3)), side=3, line=8, cex=2) 
		mtext(paste("R2 =",round(summary(fitV)$r.squared,3)), side=3, line=6, cex=2) 
		...
		mtext(paste(datVI$vegename[j]), side=1, line=4, cex=2)
	}
