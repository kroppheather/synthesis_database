library(plyr)
#read in data
setwd("c:\\Users\\hkropp\\Google Drive\\raw_data\\analysis_u7\\Tmod5p1")
...
datCS<-datC[datC$parms1=="T.aveS"|datC$parms1=="AmpSS"|datC$parms1=="AmpSC",]
...
datCS$SDS<-c(as.numeric( gsub(dexps2,"", pnames )))

datC<-data.frame(M=datCS[,1],pc2.5=datCS[,5],pc97.5=datCS[,9],param=as.character(datCS[,10]),SDWS=datCS[,11])
...
sTa1<-join(datC, datSI, by="SDWS", type="left")



#now make plots by site to see how this compares
#set up sine function
Tsinecos<-function(Tave,AmpSC,AmpSS,Tyear){
		Tave+(AmpSC*cos(2*3.14*Tyear))-(AmpSS*sin(2*3.14*Tyear))
}
...
for(i in 1:dim(sitesS)[1]){
	...
	DFP1[[i]]<-data.frame(TA=sTa1$M[sTa1$siteid==sitesS$siteid[i]&sTa1$param=="T.aveS"],
		depth=sTa1$depth[sTa1$siteid==sitesS$siteid[i]&sTa1$param=="T.aveS"],
				wyear=as.numeric(sTa1$wyear[sTa1$siteid==sitesS$siteid[i]&sTa1$param=="T.aveS"]))
				
	DFtest3[[i]]<-join(DFtest[[i]],DFP1[[i]], by=c("wyear", "depth"), type="left")
					
	DFP2[[i]]<-data.frame(AC=sTa1$M[sTa1$siteid==sitesS$siteid[i]&sTa1$param=="AmpSC"],
		depth=sTa1$depth[sTa1$siteid==sitesS$siteid[i]&sTa1$param=="AmpSC"],
				wyear=as.numeric(sTa1$wyear[sTa1$siteid==sitesS$siteid[i]&sTa1$param=="AmpSC"]))

					
	DFP3[[i]]<-data.frame(AS=sTa1$M[sTa1$siteid==sitesS$siteid[i]&sTa1$param=="AmpSS"],
		depth=sTa1$depth[sTa1$siteid==sitesS$siteid[i]&sTa1$param=="AmpSS"],
				wyear=as.numeric(sTa1$wyear[sTa1$siteid==sitesS$siteid[i]&sTa1$param=="AmpSS"]))
	...
	Ycs[[i]]<-Tsinecos(DFtest5[[i]]$TA,DFtest5[[i]]$AC,DFtest5[[i]]$AS,DFtest5[[i]]$x)
...
for(n in 1:dim(sitesS)[1]){	
	i<-sitesS$siteid[n]
	jpeg(file=paste0("c:\\Users\\hkropp\\Google Drive\\raw_data\\analysis_u7\\Tmod5p1\\plots\\site",i,".jpg"),
	...
	for(j in 1:length(depthP[[n]])){
		points(datSM$decdateA[datSM$siteid==i&datSM$depth==depthP[[n]][j]],
				...
				pch=19, col=colP[j])
		points(xP[[n]],Ycs[[n]][DFtest5[[n]]$depth==depthP[[n]][j]],
