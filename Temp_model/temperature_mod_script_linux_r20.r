...
rn <- 20
#output dir
outDir <- "/home/hkropp/synthesis/output_u10_p2/run20"
...
#this is a run for running remaining models	
#with a reduced thinning. Realized I was very
#conservative in thinning and big data models will
#take months to run
...
alreadyRunS <- list()
siteRunIDS<- list()
for(i in 1:length(alreadyRun)){
		alreadyRunS[[i]] <- list.dirs(paste0(prevDir,"/",alreadyRun[i]), full.names=FALSE,recursive=FALSE)
		#get site number
		siteRunIDS[[i]] <- data.frame(siteid=as.numeric(gsub("\\D","",alreadyRunS[[i]])),
								run=rep(as.numeric(gsub("\\D","",alreadyRun[i],length(alreadyRunS[[i]])))))
}	

sitePrevRun <- ldply(siteRunIDS,data.frame)

#join into sitesS
sitesS2 <- join(sitesS,sitePrevRun, by="siteid", type="left")

sites2Run <- sitesS2[is.na(sitesS2$run)==TRUE,]
		 
#divide into 10 runs
if(rn ==18){
	modRun <- sites2Run[1:2,]
}
if(rn ==19){
	modRun <- sites2Run[3:4,]
}
if(rn == 20){
	modRun <- sites2Run[5:6,]
}

if(rn == 21){
	modRun <- sites2Run[7:8,]
}

if(rn == 22){
	modRun <- sites2Run[9:10,]
}

if(rn == 23){
	modRun <- sites2Run[11:12,]
}

if(rn == 24){
	modRun <- sites2Run[13:14,]
}

if(rn == 25){
	modRun <- sites2Run[15:16,]
}

if(rn == 25){
	modRun <- sites2Run[17:18,]
}

if(rn == 25){
	modRun <- sites2Run[19:21,]
}
