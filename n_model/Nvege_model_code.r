...
###mean model for summer observations
	for(i in 1: NobsS){
		...
		mu.nS[i]<-nbeta1S[regIDS[i]]+nbeta2S[regIDS[i]]*EVIS[i]+nbeta3S[regIDS[i]]*OLTS[i]+nbeta4S[regIDS[i]]*shrubCS[i]+nbeta5S[regIDS[i]]*mossCS[i]+eps[yearS[i]]
	...
	for(i in 1:NobsW){
		...
		#mean n factor function of distance from sensor and location and
		#year random effects and location random effects
		mu.nW[i]<-nbeta1W[regIDW[i]]+nbeta2W[regIDW[i]]*EVIW[i]+nbeta3W[regIDW[i]]*OLTW[i]+nbeta4W[regIDW[i]]*shrubCW[i]+nbeta5W[regIDW[i]]*mossCW[i]+epsW[yearW[i]]
