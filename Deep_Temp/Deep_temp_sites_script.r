#######################################################################################
#######################################################################################
########### Script started in August 2017 by Heather Kropp                  ###########
########### Script for looking at sites with good depth resolution          ###########
########### of soil temperature measutrements                               ###########
#######################################################################################
#######################################################################################
...
### subset to only include sites with 5 or more depths of observations
### and check deepest depth of observation
DepthcountAll<-unique(data.frame(depth=SoilS$depth, wyear=SoilS$wyear, siteid=SoilS$siteid))
DepthcountLSummary<- aggregate(DepthcountAll$depth, 
						by=list(DepthcountAll$wyear,DepthcountAll$siteid), FUN="length")
DepthcountMSummary<- aggregate(DepthcountAll$depth, 
						by=list(DepthcountAll$wyear,DepthcountAll$siteid), FUN="max")

colnames(DepthcountLSummary) <- c("wyear", "siteid", "n.depth")
DepthcountLSummary$maxD <- DepthcountMSummary$x
DepthcountLSubUse<- DepthcountLSummary[DepthcountLSummary$n>=4,]
# Don't exclude sites with shallow depths yet
# Now filter SoilS by these sites
SoilS <- join(SoilS, DepthcountLSubUse, by=c("wyear","siteid"), type="inner")
...
#day of year 1 is the start of the water year so subracting 1 is it is exactly the start
...
#there is a -999 left in the data
SoilS<- SoilS[SoilS$T>-900,]

#plot(SoilS$decdate,SoilS$T,pch=19)
...
write.table(Airrepsubout,"/local/synthesis/deep_sites/run1/AirrepIDS.csv",sep=",",row.names=FALSE)
write.table(Soilrepsubout,"/local/synthesis/deep_sites/run1/SoilrepIDS.csv",sep=",",row.names=FALSE)
...
#model
write.table(Airtowriteout,"/local/synthesis/deep_sites/run1/Tair_model.csv",sep=",",row.names=FALSE)
write.table(Soiltowriteout,"/local/synthesis/deep_sites/run1/Tsoil_model.csv",sep=",",row.names=FALSE)
...
write.table(AirIDS2,"/local/synthesis/deep_sites/run1/AirIDS.csv", sep=",", row.names=FALSE)
write.table(SoilIDS2,"/local/synthesis/deep_sites/run1/SoilIDS.csv", sep=",", row.names=FALSE)

write.table(ALLSyearID,"/local/synthesis/deep_sites/run1/SoilTaveIDS_SD.csv", sep=",", row.names=FALSE)
write.table(ALLAyearID,"/local/synthesis/deep_sites/run1/AirTaveIDS_SD.csv", sep=",", row.names=FALSE)


write.table(IDnCombo,"/local/synthesis/deep_sites/run1/ncomboIDS.csv", sep=",", row.names=FALSE)
...
dir.create(paste0("/llocal/synthesis/deep_sites/run1/site",sitesS$siteid[i]))

write.table(Mod.out$statistics, paste0("/local/synthesis/deep_sites/run1/site",sitesS$siteid[i],"Temp_mod_stats.csv"),
			sep=",",row.names=TRUE)
write.table(Mod.out$quantiles, paste0("/local/synthesis/deep_sites/run1/site",sitesS$siteid[i],"Temp_mod_quant.csv"),
			sep=",",row.names=TRUE)
...
write.table(chain1,paste0("/local/synthesis/deep_sites/run1/site",sitesS$siteid[i],"chain1_coda.csv"), sep=",")
chain2<-as.matrix(codaobj.init[[2]])
write.table(chain2,paste0("/local/synthesis/deep_sites/run1/site",sitesS$siteid[i],"chain2_coda.csv"), sep=",")
chain3<-as.matrix(codaobj.init[[3]])
write.table(chain3,paste0("/local/synthesis/deep_sites/run1/site",sitesS$siteid[i],"chain3_coda.csv"), sep=",")
...
mcmcplot(codaobj.init, parms=c("T.aveA1","TminA","TmaxA","T.aveS1",
			...
			dir=paste0("/local/synthesis/deep_sites/run1/site",sitesS$siteid[i]))		
