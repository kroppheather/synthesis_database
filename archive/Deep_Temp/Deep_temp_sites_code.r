...
#likelihood for air temperature observations
	for(i in 1:NobsA){
		...
		muA[i]<-(Tstep1[i]*(T.aveA1[IDAS[i]]-((T.aveA1[IDAS[i]]-TminA[SDWA[i]])*sin(2*3.14159265*((.25/peakWA[SDWA[i]])*T.yrA[i])))))+
				...
				(Tstep3[i]*(T.aveA1[IDAE[i]]-((TmaxA[SDWA[i]]-T.aveA1[IDAE[i]])*sin(2*3.14159265*((.25/(1-peakSA[SDWA[i]]))*(T.yrA[i]-peakSA[SDWA[i]])+.75)))))
	...
	for(i in 2:NobsS){
		...
		sineS[i]<-(TstepS1[i]*(T.aveS1[IDSS[i]]-((T.aveS1[IDSS[i]]-TminS[SDWS[i]])*sin(2*3.14159265*((.25/peakWS[SDWS[i]])*T.yrS[i])))))+
				...
				(TstepS3[i]*(T.aveS1[IDSE[i]]-((TmaxS[SDWS[i]]-T.aveS1[IDSE[i]])*sin(2*3.14159265*((.25/(1-peakSS[SDWS[i]]))*(T.yrS[i]-peakSS[SDWS[i]])+.75)))))
				
		X[i]~dbern(ptemp[i])
		ptemp[i]<-(step(startflag[i])*p[i])+((1-step(startflag[i]))*pstart[startSub[i]])
		...
		sineS[1]<-(TstepS1[1]*(T.aveS1[IDSS[1]]-((T.aveS1[IDSS[1]]-TminS[SDWS[1]])*sin(2*3.14159265*((.25/peakWS[SDWS[1]])*T.yrS[1])))))+
				...
				(TstepS3[1]*(T.aveS1[IDSE[1]]-((TmaxS[SDWS[1]]-T.aveS1[IDSE[1]])*sin(2*3.14159265*((.25/(1-peakSS[SDWS[1]]))*(T.yrS[1]-peakSS[SDWS[1]])+.75)))))
	...
	for(i in 1:NSDWA){
		...
		averageTempA[i]<-sum(TempA[ASY[i]:AEY[i]])/(AEY[i]-(ASY[i]-1))
	}		
	for(i in 1:NSDWS){
		FDDS[i]<-sum(FreezeS[SSY[i]:SEY[i]])
		TDDS[i]<-sum(ThawS[SSY[i]:SEY[i]])
		DayZero[i]<-sum(X[SSY[i]:SEY[i]])
		averageTempS[i]<-sum(TempS[SSY[i]:SEY[i]])/(SEY[i]-(SSY[i]-1))
	}	
